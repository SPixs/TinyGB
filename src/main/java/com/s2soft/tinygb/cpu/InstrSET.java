...
public class InstrSET extends Instruction {
	...
	private int getBitNumber(byte opcode) {
		...
		if (upperNibble >= 0x0C) { 
			int bitIndex = 2*(upperNibble-0x0C);
	...
	public String disassemble(GBMemory memory, int address) {
		...
		return "SET " + getBitNumber(opcode) + "," + getAddressingMode(opcode).asText(new byte[0]);
	}
	...
	public int execute(byte opcode, GBCpu cpu, byte[] additionnalBytes) {
		...
		valueToReset = BitUtils.setBit(valueToReset, bitNumber, true);
